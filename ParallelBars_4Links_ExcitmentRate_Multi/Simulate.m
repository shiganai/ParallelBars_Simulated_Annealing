...
if Para.Take_Over
    Take_Over_Str = "Para_1109211341_pop_1200_gen_11855";
    load(Take_Over_Str)
    ...
    % データに関して
    Para.constants = struct(constants_HH_AE);
    Para.q0 = generate_q0(Para.constants);
    Para.Data_Set_Time = (0:5e-2:1.4)';
    Para.Joint_num = 4;
    Para.ExcitmentRate_Step_num = 10; % 1/30刻みで活性率を決める
    Para.tau_jiteisu = 100 / 1000;
    ...
    % 初期値の設定
    Para = generate_init_ExcitmentRate(Para);
%     Para = load_and_fit_init_Mdatas(Para, 'Para_1025173328_pop_2400_gen_6595', 2400);
    ...
    Para.ode_Fcn = @(t,q,ExcitmentRate_spline) ddt_ActivatingRate(t, q, Para.constants, ExcitmentRate_spline); % 積分式
    Para.ode_Event = @(t,q) Events(t,q,Para.constants); %イベントの管理
    Para.Objective_Fcn = @(time, q, breakNum, ExcitmentRate_index) ObjectiveFcn_OnlySpin(time, q, Para.constants, Para.Spin_num_Goal, breakNum, ExcitmentRate_index, Para.Data_Set_Time); % 評価方法
    %}
    ...
    Para.Spin_num_Goal = 1.5; % 評価値に関して
    Para.ode_Fcn = @(t,q,ExcitmentRate_spline) ddt_ExcitmentRate(t, q, Para.constants, ExcitmentRate_spline); % 積分式
    ...
    Para.ode_Fcn = @(t,q,ExcitmentRate_spline) ddt_ExcitmentRate(t, q, Para.constants, ExcitmentRate_spline); % 積分式
    Para.ode_Event = @(t,q) Events(t,q,Para.constants); %イベントの管理
    Para.Objective_Fcn = @(time, q, breakNum, ExcitmentRate_index) ObjectiveFcn_OnlySpin_WithJL_thWaist(time, q, Para.constants, Para.Spin_num_Goal, breakNum, ExcitmentRate_index, Para.Data_Set_Time, Para.thWaist_AtBodyUpStraight_Range); % 評価方法
    %}
    ...
    Para.ode_Fcn = @(t,q,ExcitmentRate_spline) ddt_ExcitmentRate(t, q, Para.constants, ExcitmentRate_spline); % 積分式
    Para.ode_Event = @(t,q) Events(t,q,Para.constants); %イベントの管理
    Para.Objective_Fcn = @(time, q, breakNum, ExcitmentRate_index) ObjectiveFcn_OnlySpin_ValueLimit(time, q, Para.constants, Para.Spin_num_Goal, breakNum, ExcitmentRate_index, Para.Data_Set_Time, Para.Value_phase2_Range, Para.Value_phase3_Range); % 評価方法
    %}
    ...
    Para.ode_Event = @(t,q) Events(t,q,Para.constants); %イベントの管理
    Para.Objective_Fcn = @(time, q, breakNum, ExcitmentRate_index) ObjectiveFcn_OnlySpin_NonBent_AtPhase2(time, q, Para.constants, Para.Spin_num_Goal, breakNum, ExcitmentRate_index, Para.Data_Set_Time); % 評価方法
    %}
    ...
    Para.ode_Fcn = @(t,q,ExcitmentRate_spline) ddt_ExcitmentRate(t, q, Para.constants, ExcitmentRate_spline); % 積分式
    Para.ode_Event = @(t,q) Events(t,q,Para.constants); %イベントの管理
    Para.Objective_Fcn = @(time, q, breakNum, ExcitmentRate_index) ObjectiveFcn_OnlySpin_NonStrech_AtPhase3(time, q, Para.constants, Para.Spin_num_Goal, breakNum, ExcitmentRate_index, Para.Data_Set_Time); % 評価方法
    %}
    ...
    Para.ode_Fcn = @(t,q,ExcitmentRate_spline) ddt_ExcitmentRate(t, q, Para.constants, ExcitmentRate_spline); % 積分式
    Para.ode_Event = @(t,q) Events(t,q,Para.constants); %イベントの管理
    Para.Objective_Fcn = @(time, q, breakNum, ExcitmentRate_index) ObjectiveFcn_OnlySpin_NonStrech_AtPhase3_2(time, q, Para.constants, Para.Spin_num_Goal, breakNum, ExcitmentRate_index, Para.Data_Set_Time); % 評価方法
    %}
    ...
    Para.ode_Fcn = @(t,q,ExcitmentRate_spline) ddt_ExcitmentRate(t, q, Para.constants, ExcitmentRate_spline); % 積分式
    Para.ode_Event = @(t,q) Events(t,q,Para.constants); %イベントの管理
    Para.Objective_Fcn = @(time, q, breakNum, ExcitmentRate_index) ObjectiveFcn_OnlySpin_NonStrech_AtPhase3_3(time, q, Para.constants, Para.Spin_num_Goal, breakNum, ExcitmentRate_index, Para.Data_Set_Time); % 評価方法
    %}
end
