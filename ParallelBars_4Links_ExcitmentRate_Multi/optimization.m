...
ExcitmentRate_Step_num = Para.ExcitmentRate_Step_num;
...
%     Para.Annealing_gen_Memory = Para.Annealing_gen_Memory;
%     Para.Reset_gen_Memory = Para.Reset_gen_Memory;
%     Para.Gained_gen_Memory = Para.Gained_gen_Memory; %始めを一応カウントしてElitestのindexと合わせる
%     Para.Count_Memory = Para.Count_Memory;
...
if Para.IfScat
    ...
    figure(100)
    clf('reset')
    scatter(1:pop_num, ObjectiveValueDatas_post(:,1))
    ax100 = gca;
    xlabel(ax100, '順位')
    ...
    figure(101)
    clf('reset')
    scatter(1:pop_num, ObjectiveValueDatas_post(:,5))
    ax101 = gca;
...
while true
    ...
    ExcitmentRate_post = zeros(size(ExcitmentRate));
    
    jj = 1:elite_num;
    ExcitmentRate_post(jj, :, :) = ExcitmentRate(jj, :, :);
    ...
    for kk = 1:child_num
        ...
        for mm = 1:Joint_num
            ...
            ExcitmentRate_post(child_num1,mm,:) = cat(3,ExcitmentRate(child_num1, mm, 1:Time_Cut_Points(1)),...
                ExcitmentRate(rand_num2, mm, Time_Cut_Points(1)+1:Time_Cut_Points(2)),...
                ExcitmentRate(child_num1, mm, Time_Cut_Points(2)+1:end));
        ...
        for dd = 1:Mutation_num
            ...
            % 変異の挿入
            ExcitmentRate_post(child_num1, Mut_Joint_Points, Mut_Time_Points) = (randi(2*ExcitmentRate_Step_num + 1, 1) - ExcitmentRate_Step_num - 1) / ExcitmentRate_Step_num;
        end
        
    end
    
    ifElitestMatch = isequal(ExcitmentRate(1,:,:), ExcitmentRate_post(1,:,:));
...
%     dthSwitchNumReducedIndex = (ObjectiveValueDatas_post(:,1) < dthSwitchNum_Border) .* (ObjectiveValueDatas_post(:,5) < ObjectiveValueDatas(:,5));
%     Non_Transition_Logical_Array(logical(dthSwitchNumReducedIndex)) = 0; % 遷移させるものは 0
    
    ExcitmentRate_post(Non_Transition_Logical_Array,:,:) = ExcitmentRate(Non_Transition_Logical_Array,:,:);
...
%     Para.Count_Memory(ii,:) = count;
    ...
    if ObjectiveValueDatas_post(1,1) < ObjectiveValueDatas(1,1)
        ...
        Para.Midterm_Elitest_ExcitmentRate(size(Para.Midterm_Elitest_ExcitmentRate,1) + 1,:,:) = ExcitmentRate_post(1,:,:);
...
%         no_gain_gen_num = no_gain_gen_num + 1;
    ...
    if Transition_Probability > Base_Transition_Probability
        ...
        if Annealing_gen_num >= Annealing_gen
            ...
            Para.Annealed_ActivaitingRate = ExcitmentRate_post;
    ...
    if no_gain_gen_num >= No_Gain_gen_Threshold
        ...
        Para.Before_Annealing_ExcitmentRate = ExcitmentRate_post;
