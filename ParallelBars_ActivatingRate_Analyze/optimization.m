...
if Para.IfScat
    ...
    xlabel('世代数')
    ylabel('評価値(低いほど良い)')
    ylim(ax, [max(0, ObjectiveValueDatas_post(1,1) - (322-ObjectiveValueDatas_post(1,1)) * 0.1), min(330, 322 + (322-ObjectiveValueDatas_post(1,1)) * 0.1)])
    yticks(ax, 'auto')
    ...
    figure(100)
    ax = gca;
    scatter(ax, 1:pop_num, ObjectiveValueDatas_post(:,1))
    xlabel(ax, '世代数')
    ylabel(ax, '評価値(低いほど良い)')
    drawnow
end
toc
...
while true
    ...
    if ii >= Average_Range
        Count_Mean_Memory(mod(ii,Average_Range)+1,:) = mean(Para.Count_Memory(ii-(Average_Range-1):ii,1)-Para.Count_Memory(ii-(Average_Range-1):ii,2));
    end
...
%         no_gain_gen_num = no_gain_gen_num + 1;
    ...
    if no_gain_gen_num >= No_Gain_gen_Threshold
        if Para.IfScat
            line(ax, [ii+1, ii+1], [0 330], "Color", "red")
        end
        fprintf("-----------------------------------------焼きなまし開始-----------------------------------------\n")
            ...
            if isempty(find(Annealing_gen_Memory_After_Reset(end-(Reset_Annealing_Count_Threshold-1):end,2) == 1, 1))
                ...
                if size(Para.Reset_gen_Memory, 1) >= Save_Reset_Count_Threshold
                    if isempty(find(Para.Reset_gen_Memory(end-(Save_Reset_Count_Threshold-1):end,2) == 1, 1))
                        ...
                        if Para.IfScat
                            line(ax, [ii+1, ii+1], [0 330], "Color", "green")
                        end
                        If_Gained_AfterReset = 0;
