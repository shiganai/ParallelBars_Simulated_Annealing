function ObjectiveValueData = ObjectiveFcn(time, q, constants, Spin_num_Goal, breakNum, ActivatingRate_index, Data_Set_Time, thWaist_AtBodyUpStraight_Range)
...
%     MrPB = find_MrPB(ddrPB,ddthHand,ddthWaist,ddthShoulder,dthHand,dthWaist,dthShoulder,g,kPB,mArm,mBody,mLeg,mPB,rArm,rArmMCD,rBody,rBodyMCD,rLegMCD,rPB,thHand,thShoulder,thWaist);
    ...
    MthHand_Voluntary = (MthHand - MthHand_PassiveTorque);
    MthShoulder_Voluntary = (MthShoulder - MthShoulder_PassiveTorque);
    MthWaist_Voluntary = (MthWaist - MthWaist_PassiveTorque);

    Step_Time = diff(time(1:2));
    
    Power_thHand = MthHand_Voluntary .* dthHand;
    ...
    Power_thShoulder = MthShoulder_Voluntary .* dthShoulder;
    ...
    Power_thWaist = MthWaist_Voluntary .* dthWaist;
    ...
    ActivatingRateSwitchValue = ActivatingRateSwitchValue * 1e0;
    Work_thValue = Work_thValue * 1e1;
...
elseif ~isempty(find((2:4) == breakNum, 1))
    ...
    ObjectiveValue = (-EvaluateValue(pi, pi, thShoulder(end)) + 1) * 1e2 + 211;
