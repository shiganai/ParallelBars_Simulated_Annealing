...
MDataArray = rad2deg([dthHand, dthShoulder, dthWaist]);
...
% otherDataArray = [dthHand, dthShoulder, dthWaist];
...
if exist('graphs_main','var')
    if isvalid(graphs_main)
        ...
        graphs_main = AnimeAndData_WithSticks_Resize(time, xArray, yArray, thDataArray, MDataArray, otherDataArray, sliderTimeh);
    end
else
    graphs_main = AnimeAndData_WithSticks_Resize(time, xArray, yArray, thDataArray, MDataArray, otherDataArray, sliderTimeh);
end
...
legend(graphs_main.axThData,{'手首の動員率','肩の動員率','腰の動員率'},'location','best')
...
legend(graphs_main.axMData,{'手首の角速度','肩の角速度','腰の角速度'},'location','best')
...
legend(graphs_main.axOtherData,{'腕の角度','肩角度','腰角度'},'location','best')
...
% ylim(graphs_main.axMData,[-1.1,1.1])
