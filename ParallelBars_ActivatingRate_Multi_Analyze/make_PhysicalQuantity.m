...
T = 1/2 * mPB * (vHand * vHand') ...
    ...
    + 1/2 * InertiaBody * diff(thHandpre + 1/2*pi + thShoulderpre, t)^2 ...
    + 1/2 * InertiaLeg * diff(thHandpre + 1/2*pi + thShoulderpre + thWaistpre, t)^2;

U = 1/2 * (2*kPB) * pHandf(2)^2 ...
    + mPB * g * pHandf(2) ...
    + mArm * g * pArmMCDf(2) ...
    + mBody * g * pBodyMCDf(2) ...
    + mLeg * g * pLegMCDf(2);
...
KE2 = formula(subs(KE2, SymReplaced, SymReplacing));

job5 = createJob(c);
createTask(job5, @matlabFunction, 1,{KE2, 'file', 'find_KE2.m', 'outputs', {'KE2'}});
submit(job5)
job5.Tasks
%}
