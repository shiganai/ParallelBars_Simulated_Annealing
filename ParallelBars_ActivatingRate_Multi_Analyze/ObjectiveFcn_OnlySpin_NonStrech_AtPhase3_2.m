function ObjectiveValueData = ObjectiveFcn_OnlySpin_NonStrech_AtPhase3_2(time, q, constants, Spin_num_Goal, breakNum, ActivatingRate_index, Data_Set_Time)
...
Target_Indexes = 23:25;
Target_Indexes_AR_Values = zeros([size(Target_Indexes,2), 1]) + NaN;
...
%     MrPB = find_MrPB(ddrPB,ddthHand,ddthWaist,ddthShoulder,dthHand,dthWaist,dthShoulder,g,kPB,mArm,mBody,mLeg,mPB,rArm,rArmMCD,rBody,rBodyMCD,rLegMCD,rPB,thHand,thShoulder,thWaist);
    ...
    for ii = 1:size(Target_Indexes,2)
        Target_Indexes_AR_Values(ii,1) = 1 - EvaluateValue(0, 1, ActivatingRate_index(Target_Indexes(1,ii), 3));
    end
    Target_Indexes_AR_Values = Target_Indexes_AR_Values * 100/size(Target_Indexes,2);
    
    ObjectiveValue = ObjectiveValue + sum(Target_Indexes_AR_Values, 'all');
...
ObjectiveValueData = [ObjectiveValue, spinValue, Target_Indexes_AR_Values(1,1), Target_Indexes_AR_Values(2,1), Target_Indexes_AR_Values(3,1), Target_Indexes_AR_Values(3,1), Target_Indexes_AR_Values(3,1)];
