...
% load('Para_0928195747_pop_2400_gen_5082')
% index = 330;
load('Para_1014140750_pop_2400_gen_12392')
% load('Para_1025173328_pop_2400_gen_6595')
index = 1;
...
% ActivatingRate_index(11:13,3) = 0.0;
...
% dKE = fnval(fnder(spline(time_onbar, KE)), time_onbar);
...
Torque_F_Hand = cross([pHand - pG, zeros(size(time_onbar))], [Fx_Hand, Fy_Hand, zeros(size(time_onbar))]);
Torque_F_Hand = Torque_F_Hand(:,3);
Torque_Fx_Hand = cross([pHand - pG, zeros(size(time_onbar))], [Fx_Hand, zeros(size(time_onbar)), zeros(size(time_onbar))]);
Torque_Fx_Hand = Torque_Fx_Hand(:,3);
Torque_Fy_Hand = cross([pHand - pG, zeros(size(time_onbar))], [zeros(size(time_onbar)), Fy_Hand, zeros(size(time_onbar))]);
Torque_Fy_Hand = Torque_Fy_Hand(:,3);
...
U = 1/2 * (2*kPB) * pHand(:,2).^2 ...
    + mPB * g * pHand(:,2) ...
    + mArm * g * pArmMCD(:,2) ...
    + mBody * g * pBodyMCD(:,2) ...
    + mLeg * g * pLegMCD(:,2);

dTU = fnval(fnder(spline(time_onbar, T+U)), time_onbar);
...
hold on
scatter(Data_Set_Time(Data_Set_Time<time(end)),ActivatingRate_index(Data_Set_Time<time(end),1), 'MarkerEdgeColor', line_ActivatedRate(1,1).Color)
scatter(Data_Set_Time(Data_Set_Time<time(end)),ActivatingRate_index(Data_Set_Time<time(end),2), 'MarkerEdgeColor', line_ActivatedRate(2,1).Color)
scatter(Data_Set_Time(Data_Set_Time<time(end)),ActivatingRate_index(Data_Set_Time<time(end),3), 'MarkerEdgeColor', line_ActivatedRate(3,1).Color)
hold off
...
figure(10)
clf('reset')
plot(time, Torque_F_Hand)
legend('棒反力による重心周りのトルク')
grid on

figure(11)
clf('reset')
plot(time, dmomentumG - Torque_F_Hand, time, MthHand)
legend('棒反力による重心周りのトルク', '手首のトルク')
grid on

figure(12)
clf('reset')
plot(time, dmomentumG - Torque_F_Hand - MthHand)
legend('重心周りの角運動量の微分 - 棒反力による重心周りのトルク - 手首のトルク')
xlabel('時間 [s]')
ylabel('トルク')
grid on
...
figure(13)
clf('reset')
plot(time, dP - [Fx_Hand, Fy_Hand] - (mArm + mBody + mLeg) * [0, -g])
legend('重心の運動量の微分 - 棒反力 - 重力 の x 成分', '重心の運動量の微分 - 棒反力 - 重力 の y 成分')
xlabel('時間 [s]')
ylabel('力')
grid on
...
figure(14)
clf('reset')
plot(time, Torque_Fx_Hand)
legend('棒反力 x方向 による重心周りのトルク')
xlim(time_onbar([1,end]))
grid on

figure(15)
clf('reset')
plot(time, Torque_Fy_Hand)
legend('棒反力 y方向 による重心周りのトルク')
xlim(time_onbar([1,end]))
grid on
...
figure(17)
clf('reset')
plot(time, PrPB + PthHand + PthShoulder + PthWaist, 'DisplayName', '仕事率の和')
hold on
plot(time, dKE, 'DisplayName', '力学的エネルギーの微分')
plot(time, dTU, 'DisplayName', 'T + U の微分')
hold off
legend
xlim(time_onbar([1,end]))
grid on
...
figure(18)
clf('reset')
plot(time, [PrPB, PthHand, PthShoulder, PthWaist])
legend('平行棒の粘性による散逸', '手首のトルクの仕事率', '肩のトルクの仕事率', '腰のトルクの仕事率')
xlim(time_onbar([1,end]))
grid on
