...
t14 = sin(t12);
t16 = dthHand.*t13;
KE = (InertiaArm.*dthHand.^2)./2.0+(InertiaLeg.*t3.^2)./2.0+(InertiaBody.*t2.^2)./2.0+kPB.*yHand.^2+(mPB.*(dxHand.^2+dyHand.^2))./2.0+(mLeg.*((t4+t16+rBody.*t2.*t11+rLegMCD.*t3.*t14).^2+(dyHand+t15+rLegMCD.*t3.*cos(t12)+rBody.*t2.*t10).^2))./2.0+(mArm.*((dxHand-dthHand.*rArmMCD.*t8).^2+(dyHand+dthHand.*rArmMCD.*t7).^2))./2.0+(mBody.*((dyHand+t15+rBodyMCD.*t2.*t10).^2+(t4+t16+rBodyMCD.*t2.*t11).^2))./2.0+g.*mLeg.*(t13+yHand+rBody.*t11+rLegMCD.*t14)+g.*mArm.*(yHand+rArmMCD.*t8)+g.*mPB.*yHand+g.*mBody.*(t13+yHand+rBodyMCD.*t11);
