function dotq = ddt_Ddt0_Log(t, q, constants, ActivatingRate_spline)
%DDT この関数の概要をここに記述
%   詳細説明をここに記述
...
if ~isempty(find(isnan(dotq),1))
    1+1
end
if ~isempty(find(imag(dotq),1))
    1+1
end
...
if t>0.08
    ...
    fprintf("\n")
    fprintf(num2str([MthHand_ActivatingRate, MthShoulder_ActivatingRate, MthWaist_ActivatingRate]))
    fprintf("\n")
    fprintf(num2str([MthHand, MthShoulder, MthWaist]))
    fprintf("\n")
    fprintf(num2str(dotq'))
    fprintf("\n")
    1+1;
end
end
