...
load('Para_0715130807_pop_2400_gen_1479')
index = 4;
...
MthHand_ActivatingRate_ddt0_spline = GetActivatingRate_ddt0_pp(Data_Set_Time, ActivatingRate(index,1,:));
MthShoulder_ActivatingRate_ddt0_spline = GetActivatingRate_ddt0_pp(Data_Set_Time, ActivatingRate(index,2,:));
MthWaist_ActivatingRate_ddt0_spline = GetActivatingRate_ddt0_pp(Data_Set_Time, ActivatingRate(index,3,:));

ActivatingRate_ii = [reshape(ActivatingRate(index,1,:), size(Data_Set_Time)), reshape(ActivatingRate(index,2,:), size(Data_Set_Time)), reshape(ActivatingRate(index,3,:), size(Data_Set_Time))];
...
ObjectiveValueData = ObjectiveFcn(time_onbar, q, constants, Spin_num_Goal, breakNum, ActivatingRate_ii_spline);
...
legend({'ddt0\_手首の動員率', 'ddt0\_肩の動員率', 'ddt0\_腰の動員率'})

figure(3)
plot(time, [dthHand, dthShoulder, dthWaist])
legend({'手首の角速度', '肩の角速度', '腰の角速度'})

figure(4)
plot(time, [ddthHand, ddthShoulder, ddthWaist])
legend({'手首の角加速度', '肩の角加速度', '腰の角加速度'})
