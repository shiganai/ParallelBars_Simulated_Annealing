...
momentumG = formula(mArm * cross([pArmMCD - pG, 0], [vArmMCD - vG, 0])...
    + mBody * cross([pBodyMCD - pG, 0], [vBodyMCD - vG, 0])...
    + mLeg * cross([pLegMCD - pG, 0], [vLegMCD - vG, 0]));

momentumG = momentumG(1, 3)...
...
[Coeffs, Terms] = coeffs(dmomentumG, [MrPB, MthHand, MthShoulder, MthWaist, g]);

% [Coeffs, Terms] = coeffs(ddrPBEq, [MrPB, MthHand, MthShoulder, MthWaist, g]);
% [Coeffs, Terms] = coeffs(ddrPBEq, [thHand, thShoulder, thWaist]);

%{/
dmomentumG = formula(dmomentumG);
job_tmp = createJob(c);
createTask(job_tmp, @matlabFunction, 1,{dmomentumG, 'file', 'find_dmomentumG_Torque.m', 'outputs', {'dmomentumG'}});
submit(job_tmp)
job_tmp.Tasks
%}
