...
ddthBody_tmp_Before = ddthHand + ddthShoulder;
...
ddthBody_tmp_After = ddthHand_tmp_After + ddthShoulder_tmp_After;

MthWaist_Coeffs_ddthBody = (ddthBody_tmp_After - ddthBody_tmp_Before)/Mth_StepH;
MthWaist_Contri_ddthBody = MthWaist_Coeffs_ddthBody .* MthWaist;
MthWaist_Contri_ddthBody_Integral = fnval(fnint(spline(time_onbar, MthWaist_Contri_ddthBody)),time_onbar);
...
plot(time_onbar, MthWaist_Coeffs_ddthBody)
title('胴体の角加速度への貢献係数')
xlabel('時間(s)')
xlim(time_onbar([1,end]))
ylabel('係数')
...
plot(time_onbar, MthWaist_Contri_ddthBody)
title('胴体の角加速度への貢献量')
xlabel('時間(s)')
xlim(time_onbar([1,end]))
ylabel('s^{^2}')
...
plot(time_onbar, MthWaist_Contri_ddthBody_Integral)
