...
load('Para_1112222800_pop_1200_gen_8066')
index = 1;
...
%{
ExcitmentRate(index,1, 2:3) = 0.7;
ExcitmentRate(index,1, 4:5) = 0.5;
% ExcitmentRate(index,1, 7) = 1;
ExcitmentRate(index,1, 9) = 1;
ExcitmentRate(index,1, 10) = 0.7;
ExcitmentRate(index,1, 11) = 0.8;
% ExcitmentRate(index,1, 1:11) = 1;
ExcitmentRate(index,1, 13) = 0.5;
ExcitmentRate(index, 1, 19:21) = -1;
% ExcitmentRate(index, 1, 21) = -0.6;
ExcitmentRate(index, 1, 22) = 1;

ExcitmentRate(index, 3, 5:7) = 0.2;
ExcitmentRate(index, 3, 8) = -0.1;
ExcitmentRate(index, 3, 9) = -0.5;
ExcitmentRate(index, 3, 14) = -0.8;
%}
% Para_0619064319_pop_2400_gen_7364 用 index = 1 用チューニング
%{
ExcitmentRate(index, 3, 1) = -0.0;
ExcitmentRate(index, 3, 2) = -0.1;
ExcitmentRate(index, 3, 3) = 0;
ExcitmentRate(index, 3, 4) = 0.2;
ExcitmentRate(index, 3, 5) = 0.2;
ExcitmentRate(index, 3, 6) = -0.2;

ExcitmentRate(index, 1, 20:23) = -1;

ExcitmentRate(index, 3, 16) = 0.0;
ExcitmentRate(index, 3, 17) = 0.0;
ExcitmentRate(index, 3, 18) = 0.0;
ExcitmentRate(index, 3, 19) = 0.0;
ExcitmentRate(index, 3, 20) = 0.2;
ExcitmentRate(index, 3, 21) = 0.3;
ExcitmentRate(index, 3, 22) = 0.2;
ExcitmentRate(index, 3, 23) = 0.1;
ExcitmentRate(index, 3, 24) = 0.2;

ExcitmentRate(index, 1, 24) = -1;
%}
% Para_0619143622_pop_2400_gen_9060 用 index = 1 用チューニング
%{
ExcitmentRate(index, 3, 1:10) = ExcitmentRate(index, 3, 1:10) - 0.2;
ExcitmentRate(index, 3, 1) = ExcitmentRate(index, 3, 1) - 0.3;
ExcitmentRate(index, 3, 2) = ExcitmentRate(index, 3, 2) + 0.2;
ExcitmentRate(index, 3, 3) = ExcitmentRate(index, 3, 3) - 0.1;
ExcitmentRate(index, 3, 5) = ExcitmentRate(index, 3, 5) + 0.1;
ExcitmentRate(index, 3, 6) = ExcitmentRate(index, 3, 6) - 0.1;
ExcitmentRate(index, 3, 10:12) = ExcitmentRate(index, 3, 10:12) - 0.4;
ExcitmentRate(index, 3, 10) = ExcitmentRate(index, 3, 10) - 0.1;
ExcitmentRate(index, 3, 11) = ExcitmentRate(index, 3, 11) - 0.3;
ExcitmentRate(index, 3, 12) = ExcitmentRate(index, 3, 12) - 0.1;
ExcitmentRate(index, 3, 8) = ExcitmentRate(index, 3, 8) + 0.5;
ExcitmentRate(index, 3, 7) = ExcitmentRate(index, 3, 7) + 0.4;
ExcitmentRate(index, 3, 9) = ExcitmentRate(index, 3, 9) + 0.7;
ExcitmentRate(index, 3, 10) = ExcitmentRate(index, 3, 10) + 0.6;

ExcitmentRate(index, 1, 18:23) = ExcitmentRate(index, 1, 18:23) - 2;

% ExcitmentRate(index, 1, 18:21) = ExcitmentRate(index, 1, 18:21) - 0.5;
%}
% Para_0702211636_pop_2400_gen_7088 用 index = 1 用チューニング
%{
ExcitmentRate(index, 3, 6) = -0.7;
ExcitmentRate(index, 3, 6) = -0.7;
%}
...
ExcitmentRate_index(12:13,3) = 0.0;

[MthHand_ActivatingRate_spline, MthHand_ExcitmentRate_spline] = Get_ActivatingRate_spline(Data_Set_Time, ExcitmentRate_index(:,1), odeTime, tau_jiteisu);
[MthShoulder_ActivatingRate_spline, MthShoulder_ExcitmentRate_spline] = Get_ActivatingRate_spline(Data_Set_Time, ExcitmentRate_index(:,2), odeTime, tau_jiteisu);
[MthWaist_ActivatingRate_spline, MthWaist_ExcitmentRate_spline] = Get_ActivatingRate_spline(Data_Set_Time, ExcitmentRate_index(:,3), odeTime, tau_jiteisu);
...
% hold on
% scatter(Data_Set_Time(Data_Set_Time<time(end)),ExcitmentRate(index,1,Data_Set_Time<time(end)), 'MarkerEdgeColor', line_ActivatedRate(1,1).Color)
% scatter(Data_Set_Time(Data_Set_Time<time(end)),ExcitmentRate(index,2,Data_Set_Time<time(end)), 'MarkerEdgeColor', line_ActivatedRate(2,1).Color)
% scatter(Data_Set_Time(Data_Set_Time<time(end)),ExcitmentRate(index,3,Data_Set_Time<time(end)), 'MarkerEdgeColor', line_ActivatedRate(3,1).Color)
% hold off
...
figure(5)
clf('reset')
plot(time, rad2deg([thHand, thShoulder, thWaist]), 'LineWidth', 2)
...
line_Width = 2;
figure(11)
line_Hand_ExcitmentRate = plot(time, fnval(MthHand_ExcitmentRate_spline, time), 'DisplayName', '手首の興奮水準', 'LineStyle', '--', 'LineWidth', line_Width);
line(time,  MthHand_ActivatedRate, 'DisplayName', '手首の活動水準', 'LineStyle', '-', 'Color', line_Hand_ExcitmentRate.Color, 'LineWidth', line_Width)
legend

line_Width = 2;
figure(12)
line_Shoulder_ExcitmentRate = plot(time, fnval(MthShoulder_ExcitmentRate_spline, time), 'DisplayName', '肩の興奮水準', 'LineStyle', '--', 'LineWidth', line_Width);
line(time,  MthShoulder_ActivatedRate, 'DisplayName', '肩の活動水準', 'LineStyle', '-', 'Color', line_Shoulder_ExcitmentRate.Color, 'LineWidth', line_Width)
legend

line_Width = 2;
figure(13)
line_Waist_ExcitmentRate = plot(time, fnval(MthWaist_ExcitmentRate_spline, time), 'DisplayName', '腰の興奮水準', 'LineStyle', '--', 'LineWidth', line_Width);
line(time,  MthWaist_ActivatedRate, 'DisplayName', '腰の活動水準', 'LineStyle', '-', 'Color', line_Waist_ExcitmentRate.Color, 'LineWidth', line_Width)
legend
hold on
scatter(Data_Set_Time, ExcitmentRate_index(:,3), 'DisplayName', '入力')
hold off
